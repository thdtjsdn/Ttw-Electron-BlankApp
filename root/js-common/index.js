...
window.initialize = function(){
	...
	//Create WebHook - JANDI;
	require( "../../TtwPlatform-00000--NodeJS/__Config--WebHookServer_API.js" );
	...
	require( "../../TtwPlatform-00000--NodeJS/global.REQUIRES.mongodb.js" );
		...
		require( "../../TtwPlatform-00000--NodeJS/js/mongodb/__define/000_define_global.mongodb.js" );

	//----------------------------------------------------------------------------------------------------;

	var cwd=global.process.cwd()+"/";var p0="../TtwPlatform-00000--NodeJS/";var p1=cwd+p0+"native_modules/";
	global.Lib.Ttw.importTll(p1+"OS-NetworkInformation.tll");
	global.Lib.Ttw.importTll(p1+"SyncRequest.tll");
	global.Lib.Ttw.importTll(p1+"OverrideSUtilHttp.request.tll");
	global.Lib.Ttw.importTll(p1+"MongoDBQuerySync.tll");
	global.Lib.Ttw.importTll(p1+"MongoDBTool__ExportImportCreator--Windows.tll");
	...
	return el;})().onload = function(){ window.console.log( "Initialize Libraries Complete." );
		...
		(function(){
			...
			window.b2link.util.importHTMLJS__And_Add_b2link_STATIC_UI(
				"/ui/App/index"
				, host + "/ui/App/index.html"
				, host + "/ui/App/index.js"
				, window.b2link.ui.APPEND_CHILD_TARGET_ELEMENT_ID
			);
		})();
		//*///----------[ E ] - HTML, JavaScript UI;

		//window.b2link_ui.message.message( "사전 협의 없이 스타트 버튼 구동하지 마세요 - 아이피 차단 당합니다." );
		//window.b2link_ui.message.message( "사전 협의 없이 스타트 버튼 구동하지 마세요 - 아이피 차단 당합니다." );
		//window.b2link_ui.message.message( "사전 협의 없이 스타트 버튼 구동하지 마세요 - 아이피 차단 당합니다." );
	...
	window[ "__INITIALIZE-PACKAGE__" ] = function( e, t )
	{
		...
		f0( "http://localhost:49323/js__Merge/___merge_min_all_1_0.js" );
		//----------;
		...
		//var host = window.b2link.url.getServerURL_WebServer_SCODE( "SYS0015" );
		//var host = "http://dnsproxy.shop:49323/";
		var host = "http://localhost:49323/";
